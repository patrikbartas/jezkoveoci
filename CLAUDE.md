...
The ASCII hedgehog (variants in `ascii-logo-navrh.md`, primary: `/\/\(•ᴥ•)`, footer variant: `(•ω•)/\/\/\~`) is the emerging visual identity — used in the landing-page caption and the 2% page footer, styled via the `.ascii-logo` class (monospace, `white-space: pre`). A full website rebuilt around this theme is planned; the current site is a quick fix. In HTML, escape `<` and `>` in the logos (`&lt;`, `&gt;`).
